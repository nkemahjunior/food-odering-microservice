...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  /**
     ...
     *
     * The argument, account is only passed the first time this callback is called on a new session, after the user signs in.  In subsequent calls, only token will be available.
     */
        ...
        try {
          ...
          const updatedToken: JWT = {
            ...
            expiresAt: Math.floor(
              Date.now() / 1000 + (resData.expires_in as number),
            ),
     ...
     * The token passed to this method is the token returned from the jwt callback above
     */
